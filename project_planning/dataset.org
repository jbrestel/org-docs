...
* Phase 2 (organism vs dataset; genomic sites)
   ...
   + Additional modeifications after UX 07.07.21 meeting
     ...
     [ ] Source version for genomes should be a list of most recent versions (functional, structural, etc.). Pull from history table.
     [ ] Dataset version for non-genomes should be in summary only (1) and be incorporated into DatasetHistory table (2).
     [X] Change instance of previous genomes page category (Annotation...) to new category (Genomics) in datasetRecords.xml
     [ ] External Databases Associated table: Rename table, and incorporate links (requires testing)
     ...
     [ ] References table: Try to add column for link type (search, jbrowse, record, download). Talk to Dave if necessary. At very least, order so Download is at top.
