...
* Phase 2 (organism vs dataset; genomic sites)
   ...
   + Bugs seen in QA
     ...
     [ ] VeuPathDB AllDatasets query not working
     [ ] LatestVerstion table needs outer join to datasetpresenter table (see plasmo DS_20c45d8ed1) (Bindu)
     [ ] Explore other Categories of Data links throw errors (SW)
     [ ] ExternalDatabases links should open a new window (SW)
     [ ] Genome Version in Data set release history could link to the record? (SW)
     [ ] External Resources may be better as a table? (SW)
     [ ] In Data Context, Genes and Gene count should link to search returning these genes (SW)
