...
#+begin_src plantuml :file images/eda_uml.png :exports results
  ...
  STUDY <.D.|{ ENTITYTYPEGRAPH
  ENTITYTYPE <.L.|{ ENTITYTYPEGRAPH
  STUDY <..|{ ENTITYTYPE
...
- [ ] Single cell RNASeq ??
- [ ] Systems Biology  
** IN PROGRESS Cross Study Comparison
*** New Plugin to load Study Entity Type and connect to existing EntityAttributes
- [ ] load study characteristics.  input file for study characteristics (yaml)
- [ ] load study designs from isatab??
- [ ] Variable for Study Name (add display name to study characterisitics file OR ISATab file)

*** Units
- [ ] Convert to Base Unit when needed
  - [ ] each variable needs to set a base unit else we choose one??

*** MegaStudy
- [X] Insert Study and EntityType rows for Mega Study

- [X] AttributeGraph and EntityType Graph would be for Mega Study
  - [X] study_id would be for the "mega" study.  
