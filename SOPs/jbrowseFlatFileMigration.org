...
** Track Configuration (Optional)
   The Query above will be run for each configuration and ALL features will be dumped into the same output file.
	...
	<trackConfigurations>
	 ...
	 #+begin_src example
	 [ApiCommonModel]$ grep -B 15 'alignment:GenericEndFeature' Model/lib/dst/jbrowse.dst 
	 [templateStart] 
	 ...
	 #+begin_src example
	 [ApiCommonModel]$ grep jbrowseClonedInsertEnds $PWD/* -rl
	 Model/src/main/java/org/apidb/apicommon/model/datasetInjector/ClonedInsertEnds.java
	 #+end_src

	 find all dataset presenter xmls
	 #+begin_src example
	 [ApiCommonPresenters]$ grep -A 10 ClonedInsertEnds * -r
	 <templateInjector className="org.apidb.apicommon.model.datasetInjector.ClonedInsertEnds">
	   <prop name="type">fosmid</prop>
	   <prop name="sourceIdField">source_id</prop>
	   <prop name="sourceIdJoiningRegex">(.*)[^(for|rev).ab1)]</prop>
	   <prop name="spanLengthCutoff">5000</prop>
	 ...
	 #+begin_example
	 [ApiCommonModel]$ grep 'domain:MassSpecPeptide' $PWD/*
