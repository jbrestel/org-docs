...
** Queries
   1. Each Query MUST return rows for Features and Subfeatures.
      - *NOTE*:  Previously jbrowse adaptor separated bulksubfeatures from features
   2. The query must retain a special MACRO for "$srcfeature_id".  This will be either an "na_sequence_id" or an "aa_sequence_id)
   ...
   3. Each track element MUST specify attributes for "type ["genomic","protein"] and fileSuffix ["gff", "bw", ...]
   4. Each query must return fields which corespond to GFF3 spec (order does not matter). (
...
* Script
  The script [[https://github.com/VEuPathDB/ApiCommonData/blob/master/Load/bin/jbrowseDumpAllFeatures][jbrowseDumpAllFeatures]] will loop through all tracks defined in the xml configuration and dump a file for each (per organism).  The output directory has the same structure as the workflow's webservices directory.  
    ...
    #+begin_src example
     usage:  jbrowseDumpAllFeatures $OUTPUT_DIR \\
                                    $ORG_ABBREV \\
                                    $REPLACE_EXISTING
    #+end_src
